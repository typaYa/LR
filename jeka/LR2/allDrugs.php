...
<nav class="navbar bg-body-tertiary" style="border: 1px solid #e3e2e2">
    <div class="container-fluid">
        <span class="navbar-brand mb-0 h1"><a href="addDrugs.php">Добавить</a></span>
    </div>
</nav>
...
$fetchAll = mysqli_fetch_assoc($selectAllDrugs);?>
<table class="table" style="margin-top: 30px">
    ...
    //не пашет поиск по имени, вообще не знаю почему, по поставщикам ищет
   /* $selectAllDrugs = mysqli_query($connect,"select * from `img` where `name`='$name'");*/

    $selectAllDrugs = mysqli_query($connect,"select * from `img` where `post`='$name'");
